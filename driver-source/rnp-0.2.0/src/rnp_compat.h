...
#define rnp_get_netdev_tc_txq(dev, tc) (&netdev_extended(dev)->qos_data.tc_to_txq[tc])
#endif
/*****************************************************************************/
...
#define __vlan_hwaccel_push_inside(skb) \
	__vlan_put_tag(skb, skb->vlan_tci)
...
#if (KYLIN_RELEASE_CODE <= KYLIN_RELEASE_VERSION(10,2)) || (LINUX_VERSION_CODE >= KERNEL_VERSION(4,19,64))
...
#if !(defined(CONFIG_KYLINOS_SERVER) && ( LINUX_VERSION_CODE > KERNEL_VERSION(4,19,0) ))
...
#else /* RHEL < 8.1 */
#if defined(HAVE_TC_SETUP_CLSFLOWER) && !(defined(CONFIG_KYLINOS_SERVER) && ( LINUX_VERSION_CODE > KERNEL_VERSION(4,19,0)))
