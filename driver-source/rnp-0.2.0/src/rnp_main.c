...
//extern struct rnp_info rnp_n20_info;
...
static bool rnp_alloc_mapped_page(struct rnp_ring *rx_ring, struct rnp_rx_buffer *bi,
...
static struct pci_device_id rnp_pci_tbl[] = {
	...
	{PCI_DEVICE(PCI_VENDOR_ID_MUCSE, PCI_DEVICE_ID_N400),
	 .driver_data = board_n400}, // n400 1G
#endif
	{PCI_DEVICE(PCI_VENDOR_ID_MUCSE, PCI_DEVICE_ID_N10C),
	 .driver_data = board_n10}, // n10 40G 10G
#if 0
	{PCI_DEVICE(PCI_VENDOR_ID_MUCSE, PCI_DEVICE_ID_N500),
...
static u16 udp_checksum(void *buf, struct iphdr *iph)
{
	...
	//printk("saddr %x daddr %x protocol %x len %x\n", ph4.saddr, ph4.daddr, ph4.protocol, ph4.payload_len);
...
__maybe_unused void check_udp_chksum(void *data, struct rnp_rx_buffer *rx_buffer)
{
			...
			if (rnp_test_ext_cmd(rx_desc, REV_OUTER_VLAN)) {
				...
				if (rx_ring->ring_flags & RNP_RING_STAGS_SUPPORT) {
					if (rnp_test_staterr(rx_desc, RNP_RXD_STAT_STAG)) {
						switch (rx_ring->q_vector->adapter->outer_vlan_type) {
	...
	if (netdev->features & NETIF_F_RXCSUM) {
		if (unlikely(rnp_test_staterr(rx_desc, RNP_RXD_STAT_ERR_MASK))) {
			...
			if ((!(netdev->flags & IFF_PROMISC) &&
				...
				if (unlikely(rnp_test_staterr(rx_desc, RNP_RXD_STAT_L4_MASK)
				...
				if (unlikely(rnp_test_staterr(
					...
					if ((size > 60) && (rx_desc->wb.rev1 & RNP_RX_L3_TYPE_MASK)) {
						err = true;
						//	return true;
					} else {
	...
	do {
		...
		while (count < rx_ring->rx_page_buf_nums && cleaned_count) {
			...
			dma_set_attr(DMA_ATTR_WEAK_ORDERING, &attrs);
                        ...
                        bi->page_offset = rx_ring->rx_per_buf_mem * count + rnp_rx_offset(rx_ring);
                        /* map page for use */
...
__maybe_unused static void rnp_update_rsc_stats(struct rnp_ring *rx_ring, struct sk_buff *skb)
{
...
static bool rnp_alloc_mapped_page(struct rnp_ring *rx_ring,
		struct rnp_rx_buffer *bi, union rnp_rx_desc *rx_desc,
 ...
 * chain.
 */
__maybe_unused static void rnp_dma_sync_frag(struct rnp_ring *rx_ring, struct sk_buff *skb)
{
#if defined(HAVE_STRUCT_DMA_ATTRS) && defined(HAVE_SWIOTLB_SKIP_CPU_SYNC)
	DEFINE_DMA_ATTRS(attrs);
			    ...
			    struct rnp_ring *rx_ring, int budget)
{
	...
	while (likely(total_rx_packets < budget)) {
		...
		if (rnp_check_csum_error(rx_ring, rx_desc, len, &driver_drop_packets)) {
			dev_kfree_skb_any(skb);
			cleaned_count++;
			err_packets++;
			if (err_packets + total_rx_packets > budget)
...
__maybe_unused static void rnp_check_sfp_event(struct rnp_adapter *adapter, u32 eicr)
{
	//struct rnp_hw *hw = &adapter->hw;
}
...
void rnp_configure_rx_ring(struct rnp_adapter *adapter, struct rnp_ring *ring)
{
	...
	if (ring->ring_flags & RNP_RING_SCATER_SETUP) {
		...
		ring_wr32(ring, PCI_DMA_REG_RX_SCATTER_LENGH, ((hw->max_length_current + 15) >> 4));
#endif
	}
...
//#endif
		...
		// we can fixed 2k ?
		rx_ring->rx_per_buf_mem = ALIGN((rnp_rx_offset(rx_ring) + rnp_rx_bufsz(rx_ring) + SKB_DATA_ALIGN(sizeof(struct skb_shared_info)) + RNP_RX_HWTS_OFFSET ), 1024);
#endif
...
#endif /* HAVE_INT_NDO_VLAN_RX_ADD_VID */
{
		...
		if (hw->feature_flags & RNP_VEB_VLAN_MASK_EN) {
			if (hw->ops.set_veb_vlan_mask) {
				if (hw->ops.set_veb_vlan_mask(hw, vid, hw->vfnum, true) != 0) {
					dev_err(&adapter->pdev->dev, "out of vlan entries in sriov mode \n");
#ifdef HAVE_INT_NDO_VLAN_RX_ADD_VID
					return -EACCES;
#else
					return;
	...
	if (hw->ops.set_vlan_filter) {
		hw->ops.set_vlan_filter(hw, vid, true, (sriov_flag && veb_setup));
	}
...
* @adapter: driver data
*/
__maybe_unused static void rnp_vlan_filter_disable(struct rnp_adapter *adapter)
{
	struct rnp_hw *hw = &adapter->hw;
 ...
 * @adapter: driver data
 */
__maybe_unused static void rnp_vlan_filter_enable(struct rnp_adapter *adapter)
{
	struct rnp_hw *hw = &adapter->hw;
 ...
 */
__maybe_unused static int rnp_hpbthresh(struct rnp_adapter *adapter, int pb)
{
	int marker = 0;
#if 0
	struct rnp_hw *hw = &adapter->hw;
		...
		if (!(tx_ring->ring_flags & RNP_RING_SKIP_TX_START)) {
			...
			while (head != tail) {
				...
				if ((timeout >= 100) && (timeout < 101)) {
					...
					e_info(drv, "wait tx done timeout %x %x\n", head, tail);
					// set this to hold hardware status		
					adapter->priv_flags |= RNP_PRIV_FLGA_TEST_TX_HANG;			
...
#ifdef FIX_VF_BUG
                rnp_wr_reg(adapter->io_addr_bar0 + (0x7982fc & (pci_resource_len(pdev, 0) - 1)), 0);
#endif
        break;
        default:
 ...
 * @adapter: pointer to the device adapter structure
 **/
__maybe_unused static void rnp_watchdog_flush_tx(struct rnp_adapter *adapter)
{
	int i;
...
static void rnp_update_link_to_vf(struct rnp_adapter *adapter)
{
	...
	if ((adapter->link_up_old != adapter->link_up)
		...
		if (!test_bit(__RNP_IN_IRQ, &adapter->state)) {
			if (0 == rnp_msg_post_status(adapter, PF_SET_LINK_STATUS)) {
 ...
 * @adapter: the rnp adapter structure
 **/
__maybe_unused static void rnp_sfp_link_config_subtask(struct rnp_adapter *adapter)
{
#if 0
	struct rnp_hw *hw = &adapter->hw;
	u32 speed;
...
__maybe_unused static void rnp_check_for_bad_vf(struct rnp_adapter *adapter)
{
#if 0
	int vf;
	struct rnp_hw *hw = &adapter->hw;
...
/* just modify rx itr */
__maybe_unused static void rnp_auto_itr_moderation(struct rnp_adapter *adapter)
{
	int i;
	struct rnp_ring *rx_ring;
...
__maybe_unused static void rnp_atr(struct rnp_ring *ring, struct rnp_tx_buffer *first)
{
#if 0
	struct rnp_q_vector *q_vector = ring->q_vector;
...
//
static int
rnp_mdio_read(struct net_device *netdev, int prtad, int devad, u32 addr, u32 *phy_value)
{
        int rc = -EIO;
        struct rnp_adapter *adapter = netdev_priv(netdev);
        struct rnp_hw *hw = &adapter->hw;
...
static int rnp_mii_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
	...
	if (cmd == SIOCGMIIREG) {
		// 
		ret = rnp_mdio_read(netdev, prtad, devad, mii->reg_num, &phy_value);
		if (ret < 0)
			return ret;
...
__maybe_unused static void rnp_setup_txr_prio(struct rnp_ring *tx_ring, int prio)
{
		      ...
		      u16 subdevice_id)
{
	...
	switch (device_id) {
		// not support
		case PCI_DEVICE_ID_N10:
		case PCI_DEVICE_ID_N10C:
		case PCI_DEVICE_ID_N400:
			is_wol_supported = 0;

		break;

		case PCI_DEVICE_ID_N500:
			is_wol_supported = 1;
...
#define RNP_NIC_BAR_N500 2
	...
	if (hw->hw_type == rnp_hw_n10) {
		if ((!rnp10_pfnum(hw_addr_bar0, pdev)) &&
				((adapter->pdev->device & 0xff00) == 0x1c00) &&
				(hw->fw_version >= 0x00050200)) {
			rpu_mpe_start(adapter);
		}
	}
