...
*/
	...
	// fix in n500?
	num_vf_macvlans =
		hw->num_rar_entries - (hw->max_pf_macvlans + 1 + adapter->num_vfs);
	...
	if (num_vebvlans)
		hw->vv_list = vv_list = 
			kcalloc(num_vebvlans, sizeof(struct vf_vebvlans), GFP_KERNEL);
...
rnp_set_vf_vlan(struct rnp_adapter *adapter, int add, int vid, u32 vf)
{
	...
	if (adapter->priv_flags & RNP_PRIV_FLAG_SRIOV_VLAN_MODE) {
		if (hw->ops.set_vf_vlan_mode) {
			if (hw->feature_flags & RNP_NET_FEATURE_VF_FIXED)
				hw->ops.set_vf_vlan_mode(hw, vid, vf + 1, (bool)add);	
			else 
...
__maybe_unused static void rnp_set_vmolr(struct rnp_hw *hw, u32 vf, bool aupe)
{
#if 0
...
__maybe_unused static void rnp_clear_vmvir(struct rnp_adapter *adapter, u32 vf)
{
	// struct rnp_hw *hw = &adapter->hw;
...
static int rnp_get_vf_queues(struct rnp_adapter *adapter, u32 *msgbuf, u32 vf)
{
	...
	// vf1 use ring8
		...
		if (hw->feature_flags & RNP_NET_FEATURE_VF_FIXED)
			msgbuf[RNP_VF_QUEUE_START] = vf * hw->sriov_ring_limit + hw->sriov_ring_limit;
		else 
...
void rnp_msg_task(struct rnp_adapter *adapter)
{
	...
	for (vf = 0; vf < adapter->num_vfs; vf++) {
		...
		// check flag
		if (test_and_set_bit(__VF_MBX_USED, &adapter->vfinfo[vf].status)) {
...
int rnp_msg_post_status_signle_link(struct rnp_adapter *adapter, int vf, int link_state)
{
...
static int rnp_disable_port_vlan(struct rnp_adapter *adapter, int vf)
{
	...
	if (adapter->priv_flags & RNP_PRIV_FLAG_SRIOV_VLAN_MODE) {
		if (hw->ops.set_vf_vlan_mode) {
			if (hw->feature_flags & RNP_NET_FEATURE_VF_FIXED)
				hw->ops.set_vf_vlan_mode(hw, adapter->vfinfo[vf].pf_vlan, vf + 1, false);	
			else 
				hw->ops.set_vf_vlan_mode(hw, adapter->vfinfo[vf].pf_vlan, vf, false);	
		}
...
rnp_enable_port_vlan(struct rnp_adapter *adapter, int vf, u16 vlan, u8 qos)
{
	...
	if (adapter->priv_flags & RNP_PRIV_FLAG_SRIOV_VLAN_MODE) {
		if (hw->ops.set_vf_vlan_mode) {
			if (hw->feature_flags & RNP_NET_FEATURE_VF_FIXED)
				hw->ops.set_vf_vlan_mode(hw, vlan, vf + 1, true);	
			else 
...
int rnp_ndo_set_vf_link_state(struct net_device *netdev, int vf, int state)
{
		...
		adapter->vfinfo[vf].link_state = rnp_link_state_off;
                rnp_msg_post_status_signle_link(adapter, vf, rnp_link_state_off);
                break;
        case IFLA_VF_LINK_STATE_AUTO:
                dev_info(pci_dev_to_dev(adapter->pdev),
                         "NDO set VF %d link state auto\n", vf);
		adapter->vfinfo[vf].link_state = rnp_link_state_auto;
                rnp_msg_post_status_signle_link(adapter, vf, rnp_link_state_auto);
                break;
        default:
                dev_err(pci_dev_to_dev(adapter->pdev),
