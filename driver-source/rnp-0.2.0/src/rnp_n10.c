 ...
 **/
__maybe_unused static void rnp_fdir_enable_n10(struct rnp_hw *hw, u32 fdirctrl)
{
}
 ...
 **/
__maybe_unused static u32 rnp_atr_compute_sig_hash_n10(union rnp_atr_hash_dword input,
 ...
 **/
__maybe_unused static u32 rnp_get_fdirtcpm_n10(union rnp_atr_input *input_mask)
{
#if 0
 ...
 **/
__maybe_unused static s32 rnp_read_eeprom_buffer_n10(struct rnp_hw *hw, u16 offset,
					u16 words, u16 *data)
{
 ...
 **/
__maybe_unused static s32 rnp_read_eeprom_n10(struct rnp_hw *hw, u16 offset, u16 *data)
{
	s32 ret_val = RNP_ERR_CONFIG;
 ...
 **/
__maybe_unused static enum rnp_media_type rnp_get_media_type_n10(struct rnp_hw *hw)
{
 ...
 **/
__maybe_unused static u32 rnp_get_supported_physical_layer_n10(struct rnp_hw *hw)
{
	u32 physical_layer = 0;
	return physical_layer;
}

__maybe_unused static s32 rnp_get_link_capabilities_n10(struct rnp_hw *hw,
...
void rnp_eth_set_tcp_sync_n10(struct rnp_eth_info *eth, int queue, bool flag, bool prio)
{
	if (flag) {
...
static void rnp_eth_set_vf_vlan_mode_n10(struct rnp_eth_info *eth, u16 vlan, int vf, bool enable)
{
	struct rnp_hw *hw = (struct rnp_hw *)&eth->back;
	u32 value = vlan;
	if (enable)
...
#define VXLAN_HW_ENABLE (1)
	...
	if (hw->fc.requested_mode == PAUSE_AUTO) {
		...
		} else if ((!(hw->fc.requested_mode & PAUSE_TX)) && (!(hw->fc.requested_mode & PAUSE_RX))){
...
s32 rnp_start_hw_ops_n10(struct rnp_hw *hw)
{
	...
	// ETH Registers
	//wr32(hw, RNP_ETH_ERR_MASK_VECTOR,~ETH_IGNORE_ALL_ERR);
	eth_wr32(eth, RNP10_ETH_ERR_MASK_VECTOR, INNER_L4_BIT | PKT_LEN_ERR | HDR_LEN_ERR);
...
s32 rnp_setup_mac_link_hw_ops_n10(struct rnp_hw *hw, u32 adv, u32 autoneg, u32 speed, u32 duplex)
{
        ...
        if (hw->phy.id == 0) {
                ...
                rnp_mbx_phy_read(hw, RNP_MDI_PHY_ID1_OFFSET, &value_r2);
                rnp_mbx_phy_read(hw, RNP_MDI_PHY_ID2_OFFSET, &value_r3);
	 ...
	 */
        ...
        if (!autoneg) {
                ...
                case RNP_LINK_SPEED_10_HALF:
                        ...
                        break;
                default:
                        value = RNP_MDI_PHY_SPEED_SELECT0 | RNP_MDI_PHY_SPEED_SELECT1;
...
static void rnp_set_tcp_sync_hw_ops_n10(struct rnp_hw *hw, int queue, bool flag, bool prio)
{
	struct rnp_eth_info *eth = &hw->eth;
		...
		struct ethtool_link_ksettings *ks)
{
		...
		if (phy_type == PHY_TYPE_SGMII) {
				...
				ethtool_link_ksettings_add_link_mode(
					ks, supported, 100baseT_Full);
				ethtool_link_ksettings_add_link_mode(
					ks, supported, 10baseT_Full);
	...
	// add 25G support here 
        if (supported_link & RNP_SFP_25G_SR) {
                ...
                ethtool_link_ksettings_add_link_mode(
                        ks, advertising, 25000baseSR_Full);
        }
        ...
        if (supported_link & RNP_SFP_25G_CR) {
                ...
                ethtool_link_ksettings_add_link_mode(
                        ks, advertising, 25000baseCR_Full);
        }
		...
		switch (hw->phy_type) {
				...
				case PHY_TYPE_10G_BASE_ER:
						...
						/*
						   ...
						   Autoneg); ethtool_link_ksettings_add_link_mode(ks,
						   advertising, Autoneg);
						   ethtool_link_ksettings_add_link_mode(ks, supported,
						   10000baseT_Full);
						   ethtool_link_ksettings_add_link_mode(ks, supported,
						   1000baseT_Full);
						   ethtool_link_ksettings_add_link_mode(ks, supported,
						   100baseT_Full); if (hw->speed == SPEED_10000)
				...
				case PHY_TYPE_SGMII:
						...
						ethtool_link_ksettings_add_link_mode(
							ks, supported, 10baseT_Full);
						ethtool_link_ksettings_add_link_mode(
							ks, supported, 1000baseT_Half);
						ethtool_link_ksettings_add_link_mode(
							ks, supported, 100baseT_Half);
						ethtool_link_ksettings_add_link_mode(
							ks, supported, 10baseT_Half);
						ethtool_link_ksettings_add_link_mode(
							ks, advertising, 1000baseT_Full);
						ethtool_link_ksettings_add_link_mode(
							ks, advertising, 100baseT_Full);
		...
		if (hw->speed == SPEED_10)
			ethtool_link_ksettings_add_link_mode(
				...
				case PHY_TYPE_10G_BASE_KR:
						...
						ethtool_link_ksettings_add_link_mode(
							ks, supported, 25000baseKR_Full);
						ethtool_link_ksettings_add_link_mode(
							ks, supported, 25000baseSR_Full);
						ethtool_link_ksettings_add_link_mode(
							ks, supported, 25000baseCR_Full);
...
__maybe_unused static bool fiber_unsupport(u32 supported_link, u8 phy_type)
{
        ...
        if ((phy_type == PHY_TYPE_40G_BASE_KR4) ||
                ...
                if (!(supported_link & (RNP_LINK_SPEED_40GB_FULL | RNP_LINK_SPEED_25GB_FULL)))
                        return true;
        }
		...
		const struct ethtool_link_ksettings *ks)
{
		...
		if (autoneg == AUTONEG_ENABLE) {
                ...
                if (advertising_link_speed) {
					...
					if((hw->force_speed_stat == FORCE_SPEED_STAT_DISABLED)){
								netdev_info(netdev, "advertising_link_speed is 0\n");
                                err = -EINVAL;
                                goto done;
					}
                }
		...
		/* set speed */
		err = hw->ops.setup_link(hw, advertising_link_speed, hw->autoneg, speed, hw->duplex);
		if (err) {
...
/**
 * rnp_get_settings - Get Link Speed and Duplex settings
 * @netdev: network interface device structure
...
int rnp10_set_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
        ...
        rnp_get_settings(netdev, &safe_ecmd);
        ...
        if (ecmd->eth_tp_mdix_ctrl) {
                ...
                if ((ecmd->eth_tp_mdix_ctrl != ETH_TP_MDI_AUTO) &&
                    (ecmd->autoneg != AUTONEG_ENABLE)) {
                        netdev_info(netdev->namev, "forcing MDI/MDI-X state is not supported when link speed and/or duplex are forced\n");
                        return -EINVAL;
                }
        }
        ...
        if (change || (hw->phy.autoneg_advertised != advertising_link_speed)) {
                ...
                if (err) {
                        e_info(probe, "setup link failed with code %d\n", err);
                        hw->ops.setup_link(hw, old_link_speed, true);
                }
                clear_bit(__RNP_IN_SFP_INIT, &adapter->state);
...
s32 rnp_phy_read_reg_hw_ops_n10(struct rnp_hw *hw, u32 reg_addr, u32 device_type, u16 *phy_data)
{
	s32 status = 0;
	u32 data = 0;
...
s32 rnp_phy_write_reg_hw_ops_n10(struct rnp_hw *hw, u32 reg_addr, u32 device_type, u16 phy_data)
{
	s32 status = 0;
...
void rnp_set_vf_vlan_mode_hw_ops_n10(struct rnp_hw *hw, u16 vlan, int vf, bool enable)
{
	struct rnp_eth_info *eth = &hw->eth;
...
static struct rnp_hw_operations hw_ops_n10 = {
	...
	.init_thermal_sensor_thresh = &rnp_init_thermal_sensor_thresh_hw_ops_n10,
