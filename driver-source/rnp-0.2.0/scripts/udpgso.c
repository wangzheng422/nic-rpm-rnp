...
static void run_test(struct sockaddr *addr, socklen_t alen)
{
	...
	if (cfg_do_ipv4)
		...
		fdr = socket(AF_INET6, SOCK_DGRAM, 0);
	if (fdr == -1)
	...
	if (cfg_do_ipv4)
		...
		fdt = socket(AF_INET6, SOCK_DGRAM, 0);
	if (fdt == -1)
...
static void run_test_v4(void)
{
	...
	//he=gethostbyname("1.1.20.101");
	addr.sin_family = AF_INET;
	addr.sin_port = htons(cfg_port);
	//addr.sin_addr = addr4;
	//addr.sin_addr = *((struct in_addr *)he->h_addr);
	// addr.sin_addr.s_addr = inet_addr("1.1.10.101");
	printf("strlen %ld\n",strlen(dstip));
	if(strlen(dstip)==0){
		addr.sin_addr.s_addr = inet_addr("1.1.10.101");
	}else{
...
static void run_test_v6(void)
{
	...
	if(strlen(dstip)==0){
		v6_r = inet_pton(AF_INET6, "fe80::3867:e6ff:fe40:ee41", &ipv6_addr);	
		//addr.sin6_addr = inet_pton("fe80::3867:e6ff:fe40:ee41");
	}else{
		v6_r = inet_pton(AF_INET6, dstip, &ipv6_addr);	
		//addr.sin6_addr = inet_pton(dstip);	
	}
	printf("test v6\n");
...
static void parse_opts(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "46cCmst:d:")) != -1) {
		switch (c) {
		case '4':
			cfg_do_ipv4 = true;
			break;
...
{
	if(argc==1){
		printf("An program to test udp gso:\n");
		printf("\tUsage:\n");
		printf("\targs\t-46cCmst:d:\n");
		printf("\t-4\trun ipv4 case\n");
		printf("\t-6\trun ipv6 case\tto be finished\n");
		printf("\t-c\tconnected \n");
		printf("\t-C\tconnecteless \n");
		printf("\t-m\tmessage more\n");
		printf("\t-s\tsetsockopt \n");
		printf("\t-t:\trun a certain case;need a case idx\n");
		printf("\t-d:\tmodify a dst ip, need an ip\n");
		return 0;
	}
